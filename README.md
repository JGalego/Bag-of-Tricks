...
Everything here is Python 3.9+, mostly standard library — most tricks have **zero dependencies**. The exception so far is `strawman`, which needs the [`anthropic`](https://github.com/anthropics/anthropic-sdk-python) SDK (and only to actually attack — it has a `--dry-run` that needs nothing).
